...
st.set_page_config(page_title="AI 首席選股經理人", page_icon="👨‍💼", layout="wide")
...
def get_stock_data(code):
    ...
    try:
        ...
        if count > 0:
            data["籌碼短評"] = f"外資累計 {f_sum}張，投信累計 {t_sum}張"
    except: pass
...
def extract_codes_from_image(img):
    """讓 Gemini AI 視覺大腦判讀圖片中的股票代號"""
    if GEMINI_API_KEY == "請填入你的_API_KEY":
        return [], "⚠️ 請先設定 GEMINI_API_KEY 才能呼叫 AI。"
        
    try:
        client = genai.Client(api_key=GEMINI_API_KEY)
        prompt = """請分析這張圖片，找出裡面「所有的股票代號」。請直接列出數字即可，不需要其他多餘的文字解釋。"""
        response = client.models.generate_content(model='gemini-2.5-flash', contents=[img, prompt])
        
        raw_codes = re.findall(r'\b\d{4}\b', response.text)
        return sorted(list(set(raw_codes))), None
    except Exception as e:
        return [], f"AI 視覺辨識發生錯誤: {e}"
...
def ask_ai_fund_manager(stock_data_list):
    ...
    try:
        ...
        for s in stock_data_list:
            ...
            report_str += f"   近5日籌碼: {s['籌碼短評']}\n"
            report_str += f"   重點新聞: {s['最新新聞']}\n\n"
        ...
        請根據「法人籌碼(外資與投信動向)」及「新聞題材」，從中挑選出 **【最強的 3 檔股票】** 作為買進建議。
        （挑選準則：投信或外資連續大買者優先、具備實質利多者優先）
...
# ================= 網頁主程式與排版 =================
st.title("👨‍💼 AI 首席選股經理人")
st.markdown("上傳您的名單，AI 將瞬間查閱**三大法人籌碼**與**全網新聞題材**，為您精選明日必買 Top 3！")
...
with tab2:
    ...
    if st.button("🚀 從圖片啟動 AI 診斷", use_container_width=True, key="btn_img"):
        if img_file is None:
            ...
            with st.spinner("👁️ AI 視覺正在辨識圖片中的代號..."):
